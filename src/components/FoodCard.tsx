  ...
  return (
    <article className="food-card food-hover food-animate-in bg-white border border-zinc-200 dark:bg-zinc-900 dark:border-zinc-800">
      ...
      <div className="mt-4 flex items-center justify-end gap-2">
        <button
          className="food-btn border text-sm"
          data-test-id="food-edit-btn"
          onClick={() => dispatch(openEdit(food.id))}
        >
          Edit Food
        </button>
        <button
          className="food-btn bg-red-600 text-white text-sm hover:bg-red-700"
          data-test-id="food-delete-btn"
          onClick={() => dispatch(openDelete(food.id))}
        >
          Delete Food
        </button>
      </div>
